...
async function uploadToCatbox(filePath) {
    try {
        ...
        // Kiểm tra link trả về có hợp lệ không
        if (fileLink.startsWith('https://litterbox.catbox.moe')) {
            console.log("✅ Upload thành công:", fileLink);
            return fileLink;
        }

        throw new Error(`Phản hồi từ server không phải link: ${fileLink}`);
    } catch (error) {
        console.error("❌ Lỗi Catbox:", error.message);
        // Fallback về link Action nếu upload thất bại
...
async function sendToTeams(totalJobs, fileLink) {
    ...
    // Cấu trúc Adaptive Card chuẩn để tránh lỗi hiển thị
    const teamsMessage = {
        "type": "message",
        "attachments": [
            {
            ...
            try {
                ...
                const response = await axios.get('http://api.scraperapi.com', {
                    params: {
                        ...
                        url: targetUrl,
                        country_code: 'ca',
                        render: 'true' // Vượt tường lửa Indeed hiệu quả hơn
                    },
                    timeout: 60000
                });
                ...
                $('.job_seen_beacon').each((i, el) => {
                    ...
                    if (title) {
                        // Cập nhật selector để lấy Salary và Location không bị N/A
...
runScraper();import axios from 'axios';
import XLSX from 'xlsx';
import * as cheerio from 'cheerio';
import fs from 'fs';
import FormData from 'form-data';
